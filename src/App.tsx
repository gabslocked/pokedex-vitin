  ...
  const applyFilters = useCallback(() => {
    ...
    let filtered = allPokemon.filter(p => {
      if (query) {
        ...
        if (!p.name.includes(query) && String(p.id) !== query && !paddedId.includes(query))
          return false;
      }
      if (activeGen !== null) {
        ...
        <div className="stats-bar">
          <span className="stats-count">
            {filteredList.length === allPokemon.length ? (
              ...
              <>Exibindo <strong>{filteredList.length}</strong> de{' '}<strong>{allPokemon.length}</strong> Pokémon</>
            )}
          </span>
        </div>
      </div>
